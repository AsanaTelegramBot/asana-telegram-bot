...
/**
 * Created by vovcyan on 19.03.17.
 */
public class SubscribeAllCommand extends BotCommand {
    private final AbstractStorage storage;
    private final AsanaClient asanaClient;

    @Autowired
    public SubscribeAllCommand(AbstractStorage storage, AsanaClient asanaClient) {
        super("subscribeall", "Unsubscribe from project notifications");
    ...
    private List<List<InlineKeyboardButton>> generateKeyboard(Collection<Workspace> workspaces) {
        ...
        for (Workspace workspace : workspaces) {
            ...
            button.setCallbackData("subscribeAll " + workspace.id);
